...
def multiLinear(input_size:int, output_size:int, nodes:list=None)->nn.Sequential:
    '''
    辅助快速定义模型线性层的功能函数
    n : 中间隐藏层的层数(原文为1)
    nodes : 自定义中间隐藏层的节点数
    ...
    if nodes is None:
        ...
        if len(nodes) == 1:
            ...
            seq = [nn.Linear(input_size, nodes[0]), nn.ReLU()]
...
class NegativeBinomialNet(nn.Module):
    ...
    def __init__(self, input_size, output_size, nodes:list=None):
        ...
        self.layer_p = multiLinear(input_size, output_size, nodes)
        self.layer_r = multiLinear(input_size, output_size, nodes)
...
class PoissonNet(nn.Module):
    ...
    def __init__(self, input_size, output_size, nodes:list=None):
        ...
        self.layer_w = multiLinear(input_size, output_size, nodes)
        self.layer_lamda = multiLinear(input_size, output_size, nodes)
